...
const { Sharderrors } = require('../Constants/Errors')
const Util = require('../utils/uitls');
...
class Shard extends EventEmitter {
    constructor(manager, id) {
        ...
        this.env = Object.assign({}, process.env, {
            ...
            SHARDS: this.id,
            SHARD_COUNT: this.manager.shardstospawn,
    ...
    async spawn(spawnTimeout = 30000) {
        if (this.process) throw new Error(Sharderrors.PROCESS_EXISTS, this.id);
        if (this.worker) throw new Error(Sharderrors.WORKER_EXISTS, this.id);
